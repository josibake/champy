...
public:
    ...
    bool IsTipRecent(const arith_uint256& min_chain_work, NodeSeconds current_time, std::chrono::seconds max_tip_age) const EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
    {
        const auto tip{Tip()};
        return tip &&
               tip->nChainWork >= min_chain_work &&
               tip->Time() >= current_time - max_tip_age;
    }
