...
void IpcSocketPairTest()
{
    ...
    std::atomic<bool> tip_returned{false};
    auto tip_handler{remote_mining->watchTip(uint256::ZERO, MillisecondsDouble{0}, [&async_tip_promise, &tip_returned](std::optional<interfaces::BlockRef> tip) {
        async_tip_promise.set_value({tip_returned.load(), std::move(tip)});
    })};
    tip_returned.store(true);
    (void)tip_handler;
    const auto tip_result{async_tip_promise.get_future().get()};
    BOOST_CHECK(tip_result.after_return);
    ...
    auto create_handler{remote_mining->createNewBlockAsync(node::BlockCreateOptions{}, false, [&create_promise, &create_returned](interfaces::Mining::CreateBlockResult result) {
        std::unique_ptr<interfaces::BlockTemplate> block_template;
        if (result) block_template = std::move(*result);
        create_promise.set_value({create_returned.load(), std::move(block_template)});
    })};
    create_returned.store(true);
    (void)create_handler;
    auto create_result{create_promise.get_future().get()};
    BOOST_CHECK(create_result.after_return);
    ...
    std::atomic<bool> next_returned{false};
    auto next_handler{create_result.block_template->watchNext(node::BlockWaitOptions{}, [&next_promise, &next_returned](std::unique_ptr<interfaces::BlockTemplate> block_template) {
        next_promise.set_value({next_returned.load(), std::move(block_template)});
    })};
    next_returned.store(true);
    (void)next_handler;
    auto next_result{next_promise.get_future().get()};
    BOOST_CHECK(next_result.after_return);
    ...
    auto submit_handler{next_result.block_template->submitSolutionAsync(0, 0, 0, MakeTransactionRef(CMutableTransaction{}), [&submit_promise, &submit_returned](bool accepted) {
        submit_promise.set_value({submit_returned.load(), accepted});
    })};
    submit_returned.store(true);
    (void)submit_handler;
    const auto submit_result{submit_promise.get_future().get()};
    BOOST_CHECK(submit_result.after_return);
    ...
    std::atomic<bool> check_returned{false};
    auto check_handler{remote_mining->checkBlockAsync(CBlock{}, node::BlockCheckOptions{}, [&check_promise, &check_returned](bool valid, std::string reason, std::string debug) {
        check_promise.set_value({check_returned.load(), valid, std::move(reason), std::move(debug)});
    })};
    check_returned.store(true);
    (void)check_handler;
    const auto check_result{check_promise.get_future().get()};
    BOOST_CHECK(check_result.after_return);
