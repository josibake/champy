...
class RecordingBatchView final : public Consensus::SpendStateBatchView {
public:
...
BOOST_AUTO_TEST_CASE(spend_state_batch_adapter_reads_coins_in_request_order)
{
    ...
    const Consensus::SpendStateBatchViewAdapter batch_view{spend_state};
...
BOOST_AUTO_TEST_CASE(lookup_block_spent_outputs_preserves_lookup_metadata)
{
    ...
    const std::vector results{Consensus::LookupBlockSpentOutputs(lookups, batch_view)};
