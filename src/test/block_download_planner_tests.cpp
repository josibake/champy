...
BOOST_AUTO_TEST_CASE(planner_respects_ibd_block_download_window)
{
    ...
    const auto plan{node::PlanNextBlocksToDownload({
        ...
        .limited_peer_min_blocks = 288,
        .chain = Chain(candidates),
        .ibd_window = node::IbdBlockDownloadWindow{
            .next_commit_height = 101,
            .expected_parent_hash = Block(100).hash,
            .limits = node::IbdBlockDownloadLimits{.max_blocks_ahead = 2},
        },
    })};
...
BOOST_AUTO_TEST_CASE(planner_respects_ibd_block_download_parent_boundary)
{
    std::vector candidates{Candidate(101), Candidate(102)};
