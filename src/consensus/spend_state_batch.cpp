...
std::vector<std::optional<CoinSnapshot>> SpendStateBatchViewAdapter::GetCoins(std::span<const COutPoint> outpoints) const
{
...
std::vector<BlockSpentOutputLookupResult> LookupBlockSpentOutputs(std::span<const BlockSpentOutputLookup> lookups, const SpendStateBatchView& spend_state)
{
...
BlockSpentOutputJoin JoinBlockSpentOutputs(std::span<const CTransactionRef> transactions, int block_height, const SpendStateBatchView& spend_state)
{
    ...
    for (BlockSpentOutputLookupResult& result : LookupBlockSpentOutputs(plan.external_lookups, spend_state)) {
        if (!result.coin) {
