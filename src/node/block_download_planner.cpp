...
static bool IbdBlockDownloadWindowAccepts(const std::optional<IbdBlockDownloadWindow>& window, const PeerBlockRef& block)
{
    if (!window) return true;
    return AdmitIbdBlockDownloadCandidate(*window, block).status == IbdBlockDownloadAdmissionStatus::Accepted;
}
...
BlockDownloadPlannerResult PlanNextBlocksToDownload(const BlockDownloadPlannerRequest& request)
{
    ...
    for (const BlockDownloadCandidateFacts& candidate : request.chain.candidates) {
        ...
        if (!IbdBlockDownloadWindowAccepts(request.ibd_window, candidate.block)) {
            return result;
        }
